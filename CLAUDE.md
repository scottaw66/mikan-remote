...
- **MikanProtocol** (`MikanProtocol/`) — Swift Package shared by both apps. Defines `ClientMessage` (mouseMove, mouseClick, mouseScroll, openURL, performCommand, hello, pairResponse, updateSettings, updateActions, setAudioDevice, typeText), `ServerMessage` (actionConfig, serverStatus, pairRequired, pairAccepted, pairRejected, settingsSync), and `Action` as Codable types with flat JSON encoding using a `type` discriminator field.
...
- **Page scroll buttons (iPhone):** a slim vertical pair (`chevron.up.2` / `chevron.down.2`) to the right of the trackpad in the main remote view. Each tap sends `mouseScroll(deltaX: 0, deltaY: ±750)` — deliberately reusing the pixel scroll-wheel path (delivered to whatever's under the cursor) instead of Page Up/Down keystrokes, which go to keyboard focus and die the same way the `yt*` commands do when focus lands in a text field. Positive deltaY scrolls up (natural scrolling), matching the two-finger trackpad gesture. No protocol or server changes.
- **Command buttons:** closeTab (Cmd+W), prevTab (Cmd+Shift+[), nextTab (Cmd+Shift+]), volumeUp, volumeDown, fullscreen (Ctrl+Cmd+F), arrowLeft, arrowRight, playPause (media key), escape, `return` (key 36) and `backspace` (key 51) — the last two exist for the remote keyboard.
- **Remote keyboard (iPhone):** a small keyboard icon in the bottom safe-area inset of the main remote view toggles the iOS keyboard. `RemoteKeyboardView` (`MikanRemote/RemoteKeyboardView.swift`) is a zero-size `UIKeyInput` view — **no text is stored on the phone**: each `insertText` sends `ClientMessage.typeText(text:)`, Return (arrives as `"\n"` via `insertText`) sends `performCommand("return")`, delete sends `performCommand("backspace")`. Autocorrect/autocapitalization/smart punctuation are off (no local buffer to correct against). **Any touch on the trackpad dismisses the keyboard** (`TrackpadView.onTouchBegan`); the root layout uses `.ignoresSafeArea(.keyboard)` so the trackpad stays in place under an open keyboard (the keyboard covers the action buttons instead). Server side, `MouseController.typeText` posts one key-down/up pair per Unicode scalar with `keyboardSetUnicodeString` and a dummy virtual key (0), so the payload is layout-independent; it lands wherever keyboard focus is, same caveat as the `yt*` commands. `CommandDiagnostics` logs `typeText` by character count only.
- **YouTube popup commands:** `ytPrevVideo` (Shift+P), `ytNextVideo` (Shift+N), `ytPrevChapter` (Option+Left), `ytNextChapter` (Option+Right), `ytToggleCaptions` (C), `ytSlowDown` (Shift+,), `ytSpeedUp` (Shift+.), `ytFullscreen` (F — YouTube web player only, distinct from `fullscreen`), `ytPlayPause` (K). Helper methods are on `MouseController` as `sendYouTube*`. Dispatched from `MenuBarManager.handleCommand`. The popup's last button, **YouTube Home**, sends `ytHome`, which navigates the **current tab** via `MouseController.navigateCurrentTab(to:)` — Cmd+L, type `https://www.youtube.com/`, Backspace (clears the browser's inline autocompletion so Return doesn't load a remembered `/watch?v=` URL), Return, spaced with `asyncAfter`. Deliberately not `openURL`: NSWorkspace always spawns a new tab. Browser-agnostic, no Apple Events permission needed, but like every keystroke command it assumes the browser is frontmost.
...
- **Command diagnostics logging (server):** every incoming `performCommand` is logged via unified logging (`CommandDiagnostics.swift`, category `commands`) with the frontmost app, focused window title, and focused AX element role/subrole. Purpose: the `yt*` commands are bare CGEvent keystrokes delivered to whatever has keyboard focus, so "YouTube buttons stopped working" reports (observed 2026-08-05: yt* + arrow keys dead while media keys/Cmd-shortcuts still worked, after a trackpad click on a YouTube playlist link) can be diagnosed after the fact — a focused role of `AXTextField` or a wrong frontmost window is the answer. View live: `log stream --predicate 'category == "commands"' --level default`, or filter subsystem `<BUNDLE_PREFIX>.MikanRemoteServer` in Console.app.
